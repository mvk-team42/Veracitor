...
/**
 * This file is part of cytoscape.js 2.0.0beta3-github-snapshot-2013.05.03-19.38.02.
 * 
...
}else{if(!Q){return[]}else{return[K+H*(I-K),R+H*(P-R)]}}}else{if(L==0||N==0){if([K,I,S].sort()[1]==S){return[S,M]}if([K,I,G].sort()[1]==G){return[G,O]}if([G,S,I].sort()[1]==I){return[I,P]}return[]}else{return[]}}};z.prototype.boxIntersectEllipse=function(H,Q,i,O,P,G,T,L,J){if(i<H){var I=H;H=i;i=I}if(O<Q){var R=Q;Q=O;O=R}var S=[L-G/2-P,J];var N=[L+G/2+P,J];var K=[L,J-T/2-P];var M=[L,J+T/2+P];if(i<S[0]){return false}if(H>N[0]){return false}if(Q>M[1]){return false}if(O<K[1]){return false}if(H<=N[0]&&N[0]<=i&&Q<=N[1]&&N[1]<=O){return true
}if(H<=S[0]&&S[0]<=i&&Q<=S[1]&&S[1]<=O){return true}if(H<=K[0]&&K[0]<=i&&Q<=K[1]&&K[1]<=O){return true}if(H<=M[0]&&M[0]<=i&&Q<=M[1]&&M[1]<=O){return true}H=(H-L)/(G/2+P);i=(i-L)/(G/2+P);Q=(Q-J)/(T/2+P);O=(O-J)/(T/2+P);if(H*H+Q*Q<=1){return true}if(i*i+Q*Q<=1){return true}if(i*i+O*O<=1){return true}if(H*H+O*O<=1){return true}return false};z.prototype.boxIntersectPolygon=function(ac,M,aa,J,O,V,U,S,Q,ag,T){if(aa<ac){var ab=ac;ac=aa;aa=ab}if(J<M){var L=M;M=J;J=L}var R=new Array(O.length);var ad=Math.asin(ag[1]/(Math.sqrt(ag[0]*ag[0]+ag[1]*ag[1])));
if(ag[0]<0){ad=ad+Math.PI/2}else{ad=-ad-Math.PI/2}var H=Math.cos(-ad);var G=Math.sin(-ad);for(var Z=0;Z<R.length/2;Z++){R[Z*2]=V/2*(O[Z*2]*H-O[Z*2+1]*G);R[Z*2+1]=U/2*(O[Z*2+1]*H+O[Z*2]*G);R[Z*2]+=S;R[Z*2+1]+=Q}var K=R[0];var P=R[0];var I=R[1];var N=R[1];for(var Z=1;Z<R.length/2;Z++){if(R[Z*2]>P){P=R[Z*2]}if(R[Z*2]<K){K=R[Z*2]}if(R[Z*2+1]>N){N=R[Z*2+1]}if(R[Z*2+1]<I){I=R[Z*2+1]}}if(aa<K-T){return false}if(ac>P+T){return false}if(J<I-T){return false}if(M>N+T){return false}var Y;if(T>0){var ah=B.expandPolygon(R,-T);
Y=B.joinLines(ah)}else{Y=R}for(var Z=0;Z<R.length/2;Z++){if(ac<=R[Z*2]&&R[Z*2]<=aa){if(M<=R[Z*2+1]&&R[Z*2+1]<=J){return true}}}for(var Z=0;Z<Y.length/2;Z++){var X=Y[Z*2];var W=Y[Z*2+1];var af;var ae;if(Z<Y.length/2-1){af=Y[(Z+1)*2];ae=Y[(Z+1)*2+1]}else{af=Y[0];ae=Y[1]}if(B.finiteLinesIntersect(X,W,af,ae,ac,M,aa,M,false).length>0){return true}if(B.finiteLinesIntersect(X,W,af,ae,ac,J,aa,J,false).length>0){return true}if(B.finiteLinesIntersect(X,W,af,ae,ac,M,ac,J,false).length>0){return true}if(B.finiteLinesIntersect(X,W,af,ae,aa,M,aa,J,false).length>0){return true
}}return false};z.prototype.polygonIntersectLine=function(M,L,G,J,H,P,O,N){var X=[];var K;var I=new Array(G.length);for(var T=0;T<I.length/2;T++){I[T*2]=G[T*2]*P+J;I[T*2+1]=G[T*2+1]*O+H}var S;if(N>0){var W=B.expandPolygon(I,-N);S=B.joinLines(W)}else{S=I}var R,Q,V,U;for(var T=0;T<S.length/2;T++){R=S[T*2];Q=S[T*2+1];if(T<S.length/2-1){V=S[(T+1)*2];U=S[(T+1)*2+1]}else{V=S[0];U=S[1]}K=this.finiteLinesIntersect(M,L,J,H,R,Q,V,U);if(K.length!=0){X.push(K[0],K[1])}}return X};z.prototype.shortenIntersection=function(K,J,H){var i=[K[0]-J[0],K[1]-J[1]];
var I=Math.sqrt(i[0]*i[0]+i[1]*i[1]);var G=(I-H)/I;if(G<0){return[]}else{return[J[0]+G*i[0],J[1]+G*i[1]]}};b.arrow={_points:[-0.15,-0.3,0,0,0.15,-0.3],collide:function(L,I,H,G,i,N,J,K){var M=b.arrow._points;return c.pointInsidePolygon(L,I,M,H,G,i,N,J,K)},roughCollide:function(G,M,L,I,H,i,K,J){if(typeof(b.arrow._farthestPointSqDistance)=="undefined"){b.arrow._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.arrow._points)}return c.checkInBoundingCircle(G,M,b.arrow._farthestPointSqDistance,0,H,i,L,I)
},draw:function(H){var I=b.arrow._points;for(var G=0;G<I.length/2;G++){H.lineTo(I[G*2],I[G*2+1])}},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.triangle=b.arrow;b.none={collide:function(G,M,L,I,H,i,K,J){return false},roughCollide:function(G,M,L,I,H,i,K,J){return false},draw:function(i){},spacing:function(i){return 0},gap:function(i){return 0}};b.circle={_baseRadius:0.15,collide:function(O,L,I,H,i,R,M,N){if(i!=R){var G=Math.asin(M[1]/(Math.sqrt(M[0]*M[0]+M[1]*M[1])));
...
},intersectBox:function(H,M,i,K,G,O,J,I,L){var N=p.square.points;return B.boxIntersectPolygon(H,M,i,K,N,G,O,J,I,[0,-1],L)},checkPointRough:function(G,L,K,H,i,J,I){return B.checkInBoundingBox(G,L,p.square.points,K,H,i,J,I)},checkPoint:function(G,L,K,H,i,J,I){return B.pointInsidePolygon(G,L,p.square.points,J,I,H,i,[0,-1],K)}};p.rectangle=p.square;p.octogon={};p.roundrectangle=p.square;p.roundrectangle2={roundness:4.99,draw:function(H,G,i){if(G<=roundness*2){return}B.drawPolygon(H._private.position.x,H._private.position.y,G,i,nodeSapes.roundrectangle2.points)
},intersectLine:function(I,H,G,i,J){return B.findPolygonIntersection(I,H,G,i,J,p.square.points)},intersectBox:function(H,M,i,K,G,O,J,I,L){var N=p.square.points}};p.pentagon={points:j(5,0),draw:function(G,J,I,H,i){B.drawPolygon(G,J,I,H,i,p.pentagon.points)},drawPath:function(G,J,I,H,i){B.drawPolygonPath(G,J,I,H,i,p.pentagon.points)},intersectLine:function(J,I,H,G,i,L,K){return B.polygonIntersectLine(i,L,p.pentagon.points,J,I,H/2,G/2,K)},intersectBox:function(H,M,i,K,G,O,J,I,L){var N=p.pentagon.points;
...
return(f.max<=0.5)||(f.mean<=0.3)}};function c(d){this.options=a.util.extend({},b,d)}c.prototype.run=function(){var k=this.options;var j=k.cy;var z=j.nodes();var i=j.edges();var s=j.container();var w=s.clientWidth;var u=s.clientHeight;if(j.nodes().size()<=1){if(k.fit){j.reset()}j.nodes().position({x:Math.round(w/2),y:Math.round(u/2)});j.one("layoutready",k.ready);j.trigger("layoutready");if(k.fit){j.fit(k.padding)}j.one("layoutstop",k.stop);j.trigger("layoutstop");return}var m=this.system=arbor.ParticleSystem(k.repulsion,k.stiffness,k.friction,k.gravity,k.fps,k.dt,k.precision);
...
f=+new Date}if(!p){p=true;j.one("layoutready",k.ready);j.trigger("layoutready")}}};m.renderer=t;m.screenSize(w,u);m.screenPadding(k.padding,k.padding,k.padding,k.padding);m.screenStep(k.stepSize);function o(B,C){if(C==null){return undefined}else{if(typeof C==typeof function(){}){return C.apply(B,[B._private.data,{nodes:z.length,edges:i.length,element:B}])}else{return C}}}function d(F){var I=F.x;var G=F.y;var J=w;var D=u;var C=-2;var K=2;var H=-2;var B=2;var E=4;return{x:I/J*E+C,y:G/D*E+K}}var l=function(C){grabbed=this;
var D=m.fromScreen(this.position());var B=arbor.Point(D.x,D.y);this.scratch().arbor.p=B;switch(C.type){case"grab":this.scratch().arbor.fixed=true;break;case"dragstop":this.scratch().arbor.fixed=false;this.scratch().arbor.tempMass=1000;break}};z.bind("grab drag dragstop",l);z.each(function(D,E){var G=this._private.data.id;var C=o(this,k.nodeMass);var B=this._private.locked;var F=d({x:E.position().x,y:E.position().y});this.scratch().arbor=m.addNode(G,{element:this,mass:C,fixed:B,x:B?F.x:undefined,y:B?F.y:undefined})
});i.each(function(){var E=this.id();var C=this.source().id();var D=this.target().id();var B=o(this,k.edgeLength);this.scratch().arbor=m.addEdge(C,D,{length:B})});function h(B){if(k.fit){j.fit()}B()}var e=z.filter(":grabbable");if(k.ungrabifyWhileSimulating){e.ungrabify()}var A=function(){if($.browser&&$.browser.msie){h(function(){B()})}else{B()}function B(){if(!k.liveUpdate){if(k.fit){j.reset()}j.nodes().rtrigger("position")}z.unbind("grab drag dragstop",l);if(k.ungrabifyWhileSimulating){e.grabify()
