
:mod:`database` -- Handles all database interactions
====================================================

:mod:`database.extractor` -- Extracts data from database.
---------------------------------------------------------

.. automodule:: veracitor.database.extractor
   :members:

:mod:`database.user` -- User
----------------------------
.. automodule:: veracitor.database.user
   :members:

:mod:`database.globalNetwork` -- Wrapper and help functions for the producer network.
-------------------------------------------------------------------------------------

.. automodule:: veracitor.database.globalNetwork
   :members:

:mod:`database.group` -- Group
------------------------------
.. automodule:: veracitor.database.group
   :members:

:mod:`database.information` -- Information
------------------------------------------
.. automodule:: veracitor.database.information
   :members:

:mod:`database.producer` -- Producer
------------------------------------
.. automodule:: veracitor.database.producer
   :members:

:mod:`database.tag` -- Tag
--------------------------
.. automodule:: veracitor.database.tag
   :members:

:mod:`database.dbExceptions` -- Exceptions for the database interaction
-----------------------------------------------------------------------
.. automodule:: veracitor.database.dbExceptions
   :members:
